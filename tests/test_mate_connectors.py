...
from onshape_robotics_toolkit.models.document import DefaultWorkspace, Document, DocumentMetaData, MetaWorkspaceType
...
def test_populate_mate_connectors():
    ...
    mc_data = MateConnectorFeatureData(
        ...
        mateConnectorCS=MatedCS(
            xAxis=[1.0, 0.0, 0.0], yAxis=[0.0, 1.0, 0.0], zAxis=[0.0, 0.0, 1.0], origin=[0.1, 0.2, 0.3], tf=None
        ),
        id=mc_id,
    )

    mc_feature = AssemblyFeature(
        id=mc_id, suppressed=False, featureType=AssemblyFeatureType.MATECONNECTOR, featureData=mc_data
    )
...
def test_populate_mate_connectors_in_subassembly():
    ...
    sub_occ = Occurrence(
        path=[sub_inst_id], transform=[1.0] + [0.0] * 11 + [0.0, 0.0, 0.0, 1.0], fixed=False, hidden=False
    )
    part_occ = Occurrence(
        path=[sub_inst_id, part_id], transform=[1.0] + [0.0] * 11 + [0.0, 0.0, 0.0, 1.0], fixed=False, hidden=False
    )
    ...
    mc_data = MateConnectorFeatureData(
        ...
        occurrence=[part_id],
        mateConnectorCS=MatedCS(
            xAxis=[1.0, 0.0, 0.0], yAxis=[0.0, 1.0, 0.0], zAxis=[0.0, 0.0, 1.0], origin=[0.0, 0.0, 0.0], tf=None
        ),
        id=mc_id,
    )
    ...
    root_assembly = RootAssembly(
        ...
        documentMicroversion="c" * 24,
        documentMetaData=DocumentMetaData(
            defaultWorkspace=DefaultWorkspace(id="d" * 24, type=MetaWorkspaceType.WORKSPACE), name="root", id="a" * 24
        ),
        MassProperty=None,
        isRigid=False,
        RootOccurrences=None,
    ...
    assembly = Assembly(
        ...
        parts=[],
        partStudioFeatures=[],
        document=Document(did="a" * 24, wtype="w", wid="d" * 24, eid="r" * 24, name="test"),
        name="test_assembly",
    )
...
def test_populate_mate_connectors_wrong_type():
    bad_data = MateFeatureData(matedEntities=[], mateType=MateType.FASTENED, name="Bad MC", id="M1", limits=None)
    mc_feature = AssemblyFeature(
        id="M1", suppressed=False, featureType=AssemblyFeatureType.MATECONNECTOR, featureData=bad_data
    )
    assembly = Assembly(
        rootAssembly=RootAssembly(
            instances=[],
...
def test_populate_mate_connectors_suppressed():
    ...
    mc_feature = AssemblyFeature(
        id=mc_id, suppressed=True, featureType=AssemblyFeatureType.MATECONNECTOR, featureData=mc_data
    )
    assembly = Assembly(
        rootAssembly=RootAssembly(
            instances=[part_instance],
...
def test_populate_mate_groups():
    ...
    mg_data = MateGroupFeatureData(
        name="My Mate Group", occurrences=[MateGroupFeatureOccurrence(occurrence=["part1"])], id=mg_id
    )
