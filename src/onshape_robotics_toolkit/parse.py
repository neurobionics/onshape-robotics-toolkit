...
async def traverse_instances_async(
    root: RootAssembly | SubAssembly,
    prefix: str,
    current_depth: int,
    max_depth: int,
    assembly: Assembly,
    id_to_name_map: dict[str, str],
    instance_map: dict[str, PartInstance | AssemblyInstance],
) -> None:
    """
    ...
    for instance in root.instances:
        ...
        instance_id = (
            f"{prefix}{SUBASSEMBLY_JOINER}{sanitized_name}"
            if prefix
            else sanitized_name
        )
...
def get_instances(
    ...
    dict[str, PartInstance | AssemblyInstance],
    dict[str, Occurrence],
    dict[str, str],
]:
    """
...
def get_instances_sync(
    ...
    def traverse_instances(
        root: RootAssembly | SubAssembly, prefix: str = "", current_depth: int = 0
    ) -> tuple[dict[str, PartInstance | AssemblyInstance], dict[str, str]]:
        """
        ...
        if current_depth >= max_depth:
            LOGGER.debug(
                f"Max depth {max_depth} reached. Stopping traversal at depth {current_depth}."
            )
...
def get_occurrences(
    assembly: Assembly, id_to_name_map: dict[str, str], max_depth: int = 0
) -> dict[str, Occurrence]:
    """
...
async def get_subassemblies_async(
    ...
    for instance_key, instance in instance_map.items():
        if instance.type == InstanceType.ASSEMBLY:
            if instance.isRigid:
                rigid_subassembly_instance_map.setdefault(instance.uid, []).append(
                    instance_key
                )
            else:
                subassembly_instance_map.setdefault(instance.uid, []).append(
                    instance_key
                )
    ...
    for subassembly in assembly.subAssemblies:
        ...
        if uid in subassembly_instance_map:
            is_rigid = len(subassembly.features) == 0 or all(
                feature.featureType == AssemblyFeatureType.MATEGROUP
                ...
                if is_rigid:
                    tasks.append(
                        fetch_rigid_subassemblies_async(
                            subassembly, key, client, rigid_subassembly_map
                        )
                    )
                else:
            ...
            for key in part_instance_map[part.uid]:
                tasks.append(
                    _fetch_mass_properties_async(
                        part, key, client, rigid_subassemblies, part_map
                    )
                )
...
def get_parts(
    ...
    """
    return asyncio.run(
        _get_parts_async(assembly, rigid_subassemblies, client, instances)
    )


def get_occurrence_name(
    occurrences: list[str], subassembly_prefix: str | None = None
) -> str:
    """
...
def join_mate_occurrences(
    parent: list[str], child: list[str], prefix: str | None = None
) -> str:
    """
    ...
    for feature in features:
        ...
        if feature.featureType == AssemblyFeatureType.MATE:
            ...
            try:
                child_occurrences = [
                    id_to_name_map[path]
                    for path in feature.featureData.matedEntities[CHILD].matedOccurrence
                ]
                parent_occurrences = [
                    id_to_name_map[path]
                    for path in feature.featureData.matedEntities[
                        PARENT
                    ].matedOccurrence
                ]
            ...
            if parent_occurrences[0] in rigid_subassemblies:
                _occurrence = rigid_subassembly_occurrence_map[
                    parent_occurrences[0]
                ].get(parent_occurrences[1])
                if _occurrence:
                    parent_parentCS = MatedCS.from_tf(
                        np.matrix(_occurrence.transform).reshape(4, 4)
                    )
                    parts[parent_occurrences[0]].rigidAssemblyToPartTF[
            ...
            if child_occurrences[0] in rigid_subassemblies:
                _occurrence = rigid_subassembly_occurrence_map[
                    child_occurrences[0]
                ].get(child_occurrences[1])
                if _occurrence:
                    child_parentCS = MatedCS.from_tf(
                        np.matrix(_occurrence.transform).reshape(4, 4)
                    )
                    parts[child_occurrences[0]].rigidAssemblyToPartTF[
                        child_occurrences[1]
